...
#   sed -i '' 's/?v=[0-9]*/?v=19/g' index.html
ASSET_V = 18
...
ORDER = [
    ...
    # 옐로 플라워
    # ※ R_옐로플라워두사람.png 는 넘겨받은 원본(1058x1588)에서 왼쪽 87px 을
    #    미리 덜어낸 파일입니다. 원본이 이미 2:3 이라 가로 여유가 0 이어서
    #    to_portrait 로는 좌우를 못 옮깁니다. 인물을 가운데 두려고 그 손질을
    #    파일에 박아 뒀습니다 (인물 중심 54.1% -> 50.0%).
...
FACE = [
    ...
    (0.500, 0.355),   # 12 옐로 두 사람 (왼쪽 87px 덜어낸 원본 기준)
    (0.425, 0.169),   # 13 옐로 신랑
    (0.582, 0.177),   # 14 가든 두 사람 (오른쪽 100px 덜어낸 원본 기준)
    (0.504, 0.263),   # 15 가든 소파 신부 (두 사람 컷 사이에 끼움)
