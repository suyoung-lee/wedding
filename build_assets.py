...
#   sed -i '' 's/?v=[0-9]*/?v=12/g' index.html
ASSET_V = 11
...
ORDER = [
    ...
    (N("I_가든소파두사람"), "가든 - 두 사람"),
    (N("J_가든소파두사람_시선아래"), "가든 - 두 사람"),
    (N("K_그린월두사람"),          "가든 - 잎사귀 배경"),
    (N("G_가든소파신부_정면"),     "가든 - 소파 신부"),
    (N("H_가든소파신부_시선아래"), "가든 - 소파 신부"),
    (N("L_가든소파신부_턱괸"), "가든 - 신부"),
    # 검정 배경 스튜디오 (맨 뒤)
    (A("_16"), "검정 배경 스튜디오 - 신부 정면"),
    (A("_15"), "검정 배경 스튜디오 - 신랑"),
    (N("E_검정두사람"),   "검정 배경 스튜디오 - 두 사람"),
]
...
FACE = [
    ...
    (0.520, 0.319),   # 12 옐로 두 사람
    (0.423, 0.176),   # 13 옐로 신랑
    (0.582, 0.177),   # 14 가든 두 사람 (오른쪽 100px 덜어낸 원본 기준)
    (0.568, 0.187),   # 15 가든 두 사람
    (0.516, 0.513),   # 16 가든 잎사귀 (인물이 아래쪽이라 세로 중심이 낮음)
    (0.504, 0.263),   # 17 가든 소파 신부 (정면)
    (0.505, 0.274),   # 18 가든 소파 신부 (시선 아래)
    (0.559, 0.192),   # 19 가든 신부
    (0.508, 0.210),   # 20 검정 신부
    (0.435, 0.136),   # 21 검정 신랑
    (0.505, 0.254),   # 22 검정 두 사람
]
...
COVER = N("COVER_대문사진")                   # 표지 — 아치 컷 최종 보정본
# (좌, 상, 우, 하) 비율. 좌우를 대칭으로 깎아 아치가 화면 한가운데 오게 합니다.
# 원본 1014x1522 에서 아치 바깥다리가 x=35 / x=975 (중심 505 = 이미지 중심 507)
# 이라 대칭 크롭이면 그대로 가운데입니다. 폭은 화면 비율에 가깝게 845 로.
